...
function findFirstSingleChar(str) {
  const c = str.split('');
  const nonRepeated = c.filter((char) => c.indexOf(char) === c.lastIndexOf(char))[0];
  return nonRepeated || null;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (brackets.get(stack[stack.length - 1]) === str[i]) {
      stack.pop();
    } else {
      stack.push(str[i]);
    }
...
function getCommonDirectoryPath(pathes) {
  ...
  const commonParts = splitPaths.reduce((acc, curr) => {
    let i = 0;
  ...
  return commonParts.join('/') + (commonParts.length ? '/' : '');
}
...
function evaluateTicTacToePosition(pos) {
  for (let row = 0; row < 3; row += 1) {
    if (pos[row][0] && pos[row][0] === pos[row][1] && pos[row][1] === pos[row][2]) {
      return pos[row][0];
    }
  }
  for (let col = 0; col < 3; col += 1) {
    if (pos[0][col] && pos[0][col] === pos[1][col] && pos[1][col] === pos[2][col]) {
      return pos[0][col];
    }
  }
  if (pos[0][0] && pos[0][0] === pos[1][1] && pos[1][1] === pos[2][2]) {
    return pos[0][0];
  }
  if (pos[0][2] && pos[0][2] === pos[1][1] && pos[1][1] === pos[2][0]) {
    return pos[0][2];
  }
  return undefined;
}
